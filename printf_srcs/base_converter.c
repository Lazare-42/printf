		...
		uintmax_t number, int sizeof_var, t_printf *argument)
{
	...
	while (number)
	{
		argument->argument_str->str[argument->argument_str->position++] = base_output[number % base_size];
		number /= base_size;
	}
}
		...
		intmax_t number, int sizeof_var, t_printf *argument)
{
	...
	while (number != 0)
	{
		argument->argument_str->str[argument->argument_str->position++] = base_output[(number % base_size < 0) ?
